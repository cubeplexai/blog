...
发布 CubePlex Blog 文章时，默认改用固定品牌排版，不让 AI 摘录或改写正文：

```bash
python3 scripts/cubeplex_formatter.py <article.mdx> /tmp/article_styled.html \
  --url <public-article-url> \
  --repo <blog-repository-root>
```

样式规范见 `references/CUBEPLEX_STYLE.md`。转换器直接读取当前 MDX，复制正文图片，并保留原始段落、标题和链接。除非用户明确要求一次性的特殊排版，CubePlex 文章以后都沿用该主题。
...
```bash
python3 .agents/skills/wechat-publisher/scripts/image_uploader.py <image_path>
```
...
python3 << 'EOF'
import sys
    sys.path.insert(0, '.agents/skills/wechat-publisher/scripts')
...
- **CubePlex 固定主题**: `references/CUBEPLEX_STYLE.md`
