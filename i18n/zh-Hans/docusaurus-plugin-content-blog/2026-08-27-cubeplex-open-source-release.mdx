...
今天 CubePlex 正式开源。源码已经发布在 [GitHub](https://github.com/cubeplexai/cubeplex)，采用 Apache-2.0 许可证，后端、Web 应用、部署资产和产品文档都可以直接查看和使用。
...
Sandbox 负责命令、文件、浏览器和本地进程。Agent runtime 运行在平台侧，处理模型调用、工具执行、流式响应和会话状态。底层 runtime 基于开源框架 [CubePi](https://github.com/cubeplexai/cubepi) 构建，可以接入不同模型提供方，并通过 middleware 和 checkpoint 支撑持续运行的 Agent 工作。
...
可以从 [CubePlex GitHub 仓库](https://github.com/cubeplexai/cubeplex) 开始，按照贡献说明配置本地开发环境。项目使用 Python、TypeScript、pnpm 和 Docker，提交前的检查覆盖后端、前端和端到端流程。
